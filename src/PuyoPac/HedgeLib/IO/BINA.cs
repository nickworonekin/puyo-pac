...
namespace PuyoPac.HedgeLib.IO
{
    ...
    public class BINAWriter : ExtendedBinaryWriter
    {
        ...
        public void WriteStringTable(BINAHeader header)
        {
            ...
            else if (header is PACx403Header h4)
            {
                h4.StringTableLength = (uint)BaseStream.Position - stringTablePos;
            }
        }
        ...
        public uint WriteFooter(BINAHeader header)
        {
            ...
            if (header is PACxHeader || header is PACx403Header)
                FixPadding(8);
